...
Every **tooltip ends with that control's shortcut**, and it is read from the keymap rather than
written into the button — so a control with no key shows none, and one you rebind says so
straight away (see *Keyboard shortcuts*).
...
| **Circle** | `C` | Click the centre, then a point on the rim — or press and drag the radius out. As a body: a **disk** (a one-point rounded outline — move it by its centre node, resize by its rim handle). As a cut: a **round hole** (parametric, dimensionable by diameter, patternable). As a reference: a circle whose centre snaps and measures, and whose radius **Equal** can set. |
| **Regular polygon** | `Shift+P` | Click the centre, then one corner — or press and drag; the side count shows beside the cursor (`↑` / `↓` change it). The result **stays a regular polygon**: drag a corner to grow or spin it about its centre; double-click the **"n sides" tag** above the selected polygon (or press `↑` / `↓`) to change the count; its **centre** (a small crosshair) is a snap and constraint point. In the sketch it is a rigid shape — dimension its **size** (an edge, across corners, centre → corner, centre → edge, across flats, or the horizontal / vertical span between two corners, which is what a label placed between the corners gives you) and it scales to the value, keeping its rotation; every other dimension or constraint moves or turns it whole, and a corner drag then only does what the constraints leave free. A second size on the same polygon must agree with the first, or it is refused. As a cut: a regular hole (a nut trap) with the same behaviour. |
...
| **Connect** | `Ctrl+Shift+C` | Click a joint, then another joint on a different body to **pin** them — or click a **rail** to attach the joint to it as a rider. |
...
| **Rail** | — | Click two joints on the **same body** (a moving rail), or **two free joints** (a world-fixed track — they get grounded automatically), to create a bare rail for **pin-in-slot** riders (joints that slide *and* rotate). Attach riders with Connect / the Joint tool. Rails are drawn as **double-headed arrows** spanning the travel. |
...
| **Parallel** / **Perpendicular** / **Equal** | `P` / `T` / `E` | Click **two lines** (body edges, rails or reference edges) to constrain their directions — or, for Equal, their lengths. **Equal also takes two radii**: click a disk body's rim, a round hole, a rounded corner's **arc**, a reference circle or a reference arc — any mix — and the second takes the first one's radius (a line and a circle together are refused; a sharp corner has no arc to click). Equals chain: A = B and B = C keep all three alike. One `=` badge sits at each element, red while the radii differ. Who follows: a **driving diameter / radius dimension** anywhere in the chain wins (a second dimension that disagrees is rejected, both flashing); a **reference arc** and **component geometry** can only be copied, never resized; otherwise the element you **resize directly** (rim / radius handle, `[` `]`, a reference circle's handle) is the reference, and the partners' size dimensions go back to driven like the resized outline's own. A disk that follows keeps its Tangential lines on the new rim. |
| **Fixed** | `F` | **One click, one element.** Click a **point** (joint, body corner, regular polygon centre, reference point) to lock it exactly where it is — nothing moves it again: no other constraint, no dimension, not even dragging it. Click a **line** (body edge, rail, reference edge) to lock **the line itself** — angle *and* position: its two ends stay free, but only to slide **along** that line and stretch it; the line can never turn or shift. Anything constrained to a locked element yields to it. |
...
- **A disk** — click the rim of a round hole (or a disk body) and place the label: a
  **diameter** dimension (`⌀`), drawn through the centre towards the label. In draw mode you
  ...
  reference dimension. A driving diameter also sets every radius made **Equal** to the disk.
- **A rounded corner** — click its arc and place the label: a **radius** dimension (`R`),
  ...
  **uniform** (no corner has its own radius — e.g. after a Ctrl-drag of a radius handle or
  ...
  reference. A sharp corner has no arc to click — round it a little first. A driving radius
  also sets every radius made **Equal** to the corner.
...
- **Constraints on components work the same way**: a constraint between a component and free
  ...
  between two component edges (or two component radii), or anything internal to one rigid
  piece, is refused; between a component's radius and free geometry, the free side follows. A
...
### Keyboard shortcuts
Every shortcut in this manual comes from one file. The **Shortcuts** button (the keyboard in the
**File** group) opens a small panel with three actions:

- **Export** writes `disjointed-keymap.json` — the shortcuts exactly as they are right now.
- **Import…** reads one back. Tooltips update the moment it loads, so a button that used to say
  *(J)* says whatever you gave it.
- **Reset** returns to the shortcuts Disjointed ships with.

The file is a [`keymap/1`](https://github.com/nightswimmer/KeyMapper) document — the format
**KeyMapper** reads, a board that lays every key out at once so you can drag bindings around and
see the clashes and the free letters as you go. Export from here, rearrange there, import the
result. Your keymap is kept in this browser, not in the mechanism file, so it follows you between
mechanisms and not between machines.

Things worth knowing: a command can have **more than one key** (Delete and Backspace both delete)
or **none at all** — about twenty commands ship unbound, including Export, the mirrors, the grid
and snap toggles, zoom, and the theme, and a keymap can give any of them a key. `Esc` is
hardwired. Shortcuts that answer while you are typing in a field are only Save, Save as, Open and
`F1`; everything else leaves your typing alone.
...
**Equal between radii** is not a solver item at all: a radius is a parameter, never a variable,
so the equality is re-applied after every solve the way driving diameter / radius dimensions
are — every radius linked through Equals is written to one value, taken from whoever cannot
change: a driving size dimension first, then a reference arc (three points the solver reshapes)
or component geometry, then the element you just resized or picked first. Two of the same rank
that disagree are a conflict, and the edit is rejected with them flashing.
